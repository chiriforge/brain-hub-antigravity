...
export class ChatWebviewPanel {
  ...
  public static createOrShow(extensionUri: vscode.Uri, session: ChatSession): ChatWebviewPanel {
    ...
    if (session.path) {
      localRoots.push(vscode.Uri.file(session.path));
    }
  ...
  private async handleSaveImageAs(rawSrc?: string): Promise<void> {
    ...
    try {
      ...
      const defaultDir = this.currentSession.workspacePath || this.currentSession.path || '';
  ...
  private generateHtml(session: ChatSession, messages: ChatMessage[], threadSessions: ChatSession[] = []): string {
    ...
    const hasArtifacts = session.hasArtifacts || totalArtifacts > 0;
    ...
    let artifactsHtml = '';
    if (session.hasArtifacts || totalArtifacts > 0) {
      artifactsHtml = `
    ...
    const serializedArtifacts = (session.artifacts || []).map((art) => ({
      name: art.name,
    ...
    return `
      ...
      <body data-workspace-path="${MarkdownRenderer.escapeHtml(session.workspacePath || '')}">
        ...
        <!-- Centered Artifacts Quick Viewer Modal -->
        ...
        <script>
          ...
          window.sessionArtifacts = ${JSON.stringify(serializedArtifacts)};
